...
export function ScoreHero({ match }) {
  ...
  return (
    <div className="border-[3px] border-black bg-black p-4 text-center text-white shadow-brutal-sm">
...
export function FootballBoard({ match }) {
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="border-[3px] border-black bg-white p-3 shadow-brutal-xs">
        <p className="font-black text-sm uppercase">Timeline</p>
        <div className="mt-2 flex flex-col gap-1.5">
          {(match.events || []).length ? (
            match.events.map((e, i) => (
              <div key={i} className="flex gap-2 border-2 border-black/10 bg-brutal-cream p-2 text-sm font-bold">
                <span className="shrink-0 border-2 border-black bg-black px-1.5 font-mono text-xs text-white">{e.min}</span>
                <span>{e.text}</span>
              </div>
            ))
          ) : (
            <p className="font-mono text-xs font-bold uppercase text-black/50">No events yet — kick-off soon.</p>
          )}
        </div>
      </div>
      {(match.stats || []).length > 0 && (
        <div className="border-[3px] border-black bg-brutal-yellow p-3 shadow-brutal-xs">
          <p className="font-black text-sm uppercase">Stats</p>
          {match.stats.map((s, i) => (
            <div key={i} className="mt-1 flex items-center justify-between border-2 border-black bg-white px-2 py-1 text-sm font-bold">
              <span>{s.label}</span>
    ...
    <div className="flex flex-col gap-3">
      ...
      {match.meta?.crr && (
        <div className="grid grid-cols-2 gap-2 text-center">
          ...
          <div className="border-[3px] border-black bg-white p-2 shadow-brutal-xs">
          ...
          <div className="mt-2 flex gap-1.5">
            {match.recentBalls.map((b, i) => (
              <motion.span
                ...
                className={`grid h-9 w-9 place-items-center border-[3px] border-black font-black ${b === 'W' ? 'bg-brutal-red text-white' : b === '6' || b === '4' ? 'bg-brutal-mint' : 'bg-white'}`}
              >
                {b}
              </motion.span>
            ))}
      ...
      {(match.batters || []).length > 0 && (
        <div className="border-[3px] border-black bg-white p-3 shadow-brutal-xs">
          ...
          {match.batters.map((b, i) => (
            <div key={i} className="mt-1 flex justify-between border-b-2 border-dashed border-black/20 py-1 text-sm font-bold">
              <span>{b.name}</span>
      ...
      {d.score && (
        <div className="border-[3px] border-dashed border-black bg-brutal-cream p-3 font-mono text-xs font-bold">
          {(d.score || []).map((s, i) => (
